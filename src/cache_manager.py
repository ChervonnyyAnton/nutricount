...
class CacheManager:
    ...
    def __init__(self, redis_url: str = "redis://localhost:6379/0",
        ...
        if REDIS_AVAILABLE:
                ...
                logger.warning("Redis not available, using fallback cache: {e}")
                self.use_redis = False
            else:
    ...
    def health_check(self) -> bool:
        ...
        try:
            ...
            logger.error("Cache health check error: {e}")
            return False
...
def cached(timeout: int = 300, key_prefix: str = ""):
    ...
    def decorator(func):
        @wraps(func)
            ...
            # Generate cache key
            cache_key = "{key_prefix}:{func.__name__}:{hash(str(args) + str(sorted(kwargs.items())))}"
            ...
            logger.debug("Cached result for {cache_key}")
