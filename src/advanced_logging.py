...
class StructuredLogger:
    ...
    def __init__(self, app_name: str = "nutrition-tracker",
                     log_level: str = "INFO",
    ...
    def _setup_elasticsearch(self):
        ...
        if ELASTICSEARCH_AVAILABLE and self.elasticsearch_url:
            try:
                ...
                self.logger.warning("Elasticsearch setup failed: {e}")
                    self.es_client = None
    ...
    def log_application_event(self, level: str, message: str, **kwargs):
        ...
        if LOGURU_AVAILABLE:
            ...
            self.logger.log(getattr(logging, level), "{message} | {json.dumps(extra_data)}")
        ...
        message = "{method} {path} {status_code} {duration:.3f}s"

            if LOGURU_AVAILABLE:
            self.logger.bind(**extra_data).info(message)
        else:
                self.logger.info("{message} | {json.dumps(extra_data)}")
    ...
    def log_security_event(self, event_type: str, message: str,
                              user_id: int = None, ip: str = None, **kwargs):
        """Log security event"""
        extra_data = {
                "log_type": "security",
        ...
        if LOGURU_AVAILABLE:
            ...
            self.logger.warning("{message} | {json.dumps(extra_data)}")
        ...
        message = "Performance: {operation} took {duration:.3f}s"
    ...
    def _send_to_elasticsearch(self, log_type: str, level: str, message: str, data: Dict[str, Any]):
        ...
        if not self.es_client:
            ...
            try:
            index_name = "{self.app_name}-{log_type}-{datetime.now(timezone.utc).strftime('%Y.%m.%d')}"
            ...
            # Generate alerts
                ...
                security_alerts["alerts"].append({
                    "type": "high_failed_logins",
                        "message": "High number of failed login attempts: {security_events['failed_logins']}",
                    "severity": "warning"
                })
                ...
                security_alerts["alerts"].append({
                    "type": "rate_limit_exceeded",
                        "message": "Multiple rate limit hits: {security_events['rate_limit_hits']}",
                    "severity": "info"
                })
