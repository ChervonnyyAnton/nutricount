...
class StructuredLogger:
    ...
    def _send_to_elasticsearch(self, log_type: str, level: str, message: str, data: Dict[str, Any]):
        ...
        try:
            index_name = f"{self.app_name}-{log_type}-{datetime.now(timezone.utc).strftime('%Y.%m.%d')}"
        ...
        try:
            ...
            if security_events["failed_logins"] > 10:
                security_alerts["alerts"].append({
                    ...
                    "message": f"High number of failed login attempts: {security_events['failed_logins']}",
                    "severity": "warning"
                })
            ...
            if security_events["rate_limit_hits"] > 5:
                security_alerts["alerts"].append({
                    ...
                    "message": f"Multiple rate limit hits: {security_events['rate_limit_hits']}",
                    "severity": "info"
                })
