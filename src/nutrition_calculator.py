...
def validate_nutrition_data(
    ...
    if fiber is not None:
        if fiber > carbs:
                issues.append("Клетчатка {fiber:.1f}г больше углеводов {carbs:.1f}г")
        elif fiber < 0:
    ...
    if sugars is not None:
        if sugars > carbs:
                issues.append("Сахара {sugars:.1f}г больше углеводов {carbs:.1f}г")
        elif sugars < 0:
    ...
    if sugars is not None and fiber is not None:
        if (sugars + fiber) > carbs * 1.2:
            ...
            issues.append("Сахара + клетчатка больше углеводов на {excess_percent:.0f}%")

    # Проверка общей суммы БЖУ
        if check_total:
        total_macros = protein + fats + carbs
        if total_macros > 110:  # С учетом золы и воды
                issues.append("Сумма БЖУ {total_macros:.1f}г превышает разумный предел")
    ...
    # Проверка калорий
        ...
        calorie_diff = abs(calories - calculated_calories)
            ...
            issues.append("Калории расходятся на {calorie_diff:.0f} ккал ({calorie_diff/calories*100:.0f}%)")
...
def calculate_calories_from_macros(protein: float, fats: float, carbs: float) -> float:
    ...
    if not validation.valid:
        logger.warning("Nutrition data validation issues: {validation.issues}")

        calories = (
        protein * CALORIES_PER_GRAM["protein"] + fats * CALORIES_PER_GRAM["fats"] + carbs * CALORIES_PER_GRAM["carbs"]
    )

        logger.info("Calories calculation: protein={protein}, fats={fats}, carbs={carbs} -> {calories} kcal")
...
def calculate_net_carbs_advanced(
    ...
    if fiber is not None:
        ...
        net_carbs = total_carbs - (fiber * deduction_coeff)
            ...
            "fiber_estimated": False,
                "estimation_method": "direct_fiber_data_coeff_{deduction_coeff}",
    ...
    if category and category in FIBER_RATIOS:
        ...
        net_carbs = total_carbs - (estimated_fiber * deduction_coeff)
            estimation_method = "category_{category}_ratio_{fiber_ratio}_coeff_{deduction_coeff}"
    else:
        ...
        logger.info("Net carbs calculation: total_carbs={total_carbs}, category={category} -> net_carbs={net_carbs}")
...
def calculate_keto_index_advanced(
    ...
    logger.info(
            "Advanced keto index calculation: protein={protein}, fats={fats}, carbs={carbs} -> keto_index={keto_index}"
    )
    return result
...
def calculate_bmr_mifflin_st_jeor(weight: float, height: float, age: int, gender: str) -> float:
    ...
    validation = validate_user_profile(weight, height, age, gender)
        if not validation.valid:
        raise ValueError("Invalid user profile: {validation.issues}")
    ...
    if gender == "male":
        ...
        logger.info(
        "BMR calculation (Mifflin-St Jeor): weight={weight}, height={height}, age={age}, gender={gender} -> {bmr} kcal"
    )
        return round(bmr, 0)
...
def calculate_bmr_katch_mcardle(lean_body_mass: float) -> float:
    ...
    bmr = 370 + (21.6 * lean_body_mass)
        logger.info("BMR calculation (Katch-McArdle): LBM={lean_body_mass} -> {bmr} kcal")
    return round(bmr, 0)
...
def calculate_lean_body_mass(weight_kg: float, body_fat_percentage: float) -> float:
    ...
    lbm = weight_kg * (1 - body_fat_percentage / 100)
        ...
        logger.info("LBM calculation: weight={weight_kg}, body_fat={body_fat_percentage}% -> LBM={lbm} kg")
    return round(lbm, 2)
...
def calculate_tdee(bmr: float, activity_level: Union[str, ActivityLevel]) -> float:
    ...
    if activity_level not in ACTIVITY_MULTIPLIERS:
            raise ValueError("Неизвестный уровень активности: {activity_level}")

    tdee = bmr * ACTIVITY_MULTIPLIERS[activity_level]
        logger.info("TDEE calculation: BMR={bmr}, activity={activity_level} -> {tdee} kcal")
    return round(tdee, 0)
...
def calculate_target_calories(tdee: float, goal: Union[str, Goal]) -> float:
    ...
    if goal not in GOAL_ADJUSTMENTS:
            raise ValueError("Неизвестная цель: {goal}")

    target_calories = tdee * GOAL_ADJUSTMENTS[goal]
        logger.info("Target calories calculation: TDEE={tdee}, goal={goal} -> {target_calories} kcal")
...
def calculate_keto_macros_advanced(
    ...
    logger.info(
            "Advanced keto macros calculation: target_calories={target_calories}, keto_type={keto_type} -> {result}"
    )
    return result
    ...
    logger.info("GKI calculation: glucose={glucose_mgdl}, ketones={ketones_mgdl} -> GKI={gki}")
        return result
...
def calculate_recipe_nutrition(ingredients: List[RecipeIngredient], recipe_name: str, servings: int = 1) -> Dict:
    ...
    for ingredient in ingredients:
            # Коэффициент выхода
        yield_factor = COOKING_YIELD_FACTORS.get("{ingredient.category}_{ingredient.preparation}", 1.0)
    ...
    nutrition_per_100g["fiber"] = round(fiber_per_100g, 1)
        ...
        logger.info("Recipe calculation: {recipe_name} -> {result}")
    return result
...
def validate_recipe_integrity(recipe_data: Dict) -> ValidationResult:
    ...
    if abs(calculated_raw_weight - stated_raw_weight) > 1:
        issues.append(
                "Несоответствие веса сырых ингредиентов: {calculated_raw_weight:.0f}г vs {stated_raw_weight:.0f}г"
        )
    ...
    stated_nutrition = recipe_data["nutrition_total"]
        ...
        stated_val = stated_nutrition[nutrient]
            ...
            issues.append("Несоответствие {nutrient}: {calc_val:.1f} vs {stated_val:.1f}")
    ...
    overall_yield = total_cooked / total_raw if total_raw > 0 else 0
        ...
        issues.append("Необычный коэффициент выхода: {overall_yield:.2f}")
...
def log_calculation(func_name: str, inputs: Dict, result: Dict) -> None:
    """Логирование расчетов для отладки"""
        logger.info("{func_name}: inputs={inputs}, result={result}")
...
def example_avocado_analysis():
    ...
    print("Чистые углеводы: {net_carbs_result['net_carbs']} г")
        print("Кето-индекс: {keto_result['keto_index']} - {keto_result['keto_category']}")
    print(
        "Компоненты: углеводы={keto_result['carbs_score']}, жиры={keto_result['fat_score']}, качество={keto_result['quality_score']}, ГИ={keto_result['gi_score']}"
        )
...
def example_daily_macros():
    ...
    print("BMR: {bmr} ккал")
    print("TDEE: {tdee} ккал")
        print("Целевые калории: {target_calories} ккал")
    print("Углеводы: {macros['carbs']}г ({macros['carbs_percentage']}%)")
    print("Белки: {macros['protein']}г ({macros['protein_percentage']}%)")
        print("Жиры: {macros['fats']}г ({macros['fats_percentage']}%)")
    print("LBM использована: {macros['lbm_used']}")
    ...
    print("Рецепт: {recipe['recipe_name']}")
        print("Порций: {recipe['servings']}")
    print("Вес сырой: {recipe['weights']['total_raw']}г")
    print("Вес готовый: {recipe['weights']['total_cooked']}г")
        print("Коэффициент выхода: {recipe['weights']['yield_factor']}")
    print(
        "На порцию: белки {recipe['nutrition_per_serving']['protein']}г, жиры {recipe['nutrition_per_serving']['fats']}г, углеводы {recipe['nutrition_per_serving']['carbs']}г"
        )
    print("Кето-индекс: {recipe['keto_index']} - {recipe['keto_category']}")
    print("Валидация: {'ОК' if validation.valid else validation.issues}")
