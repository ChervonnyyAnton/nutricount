...
from datetime import datetime
            ...
            if not backup_path:
                ...
                backup_path = "backups/nutrition_{timestamp}.db"

            shutil.copy2(Config.DATABASE, backup_path)
                logger.info("Database backed up to {backup_path}")
            return "sync_backup_{datetime.now().timestamp()}"
        except Exception as e:
    ...
    def _optimize_database_sync(self) -> str:
        ...
        try:
            ...
            return "sync_optimize_{datetime.now().timestamp()}"
            except Exception as e:
        ...
        try:
            ...
            logger.info("Calculating nutrition stats for {date_range}")
            return "sync_stats_{datetime.now().timestamp()}"
            except Exception as e:
            logger.error("Stats calculation error: {e}")
            raise

        def _export_data_sync(self, export_format: str) -> str:
        """Synchronous data export"""
        try:
                logger.info("Exporting data in {export_format} format")
            return "sync_export_{datetime.now().timestamp()}"
        except Exception as e:
    ...
    def _cleanup_old_logs_sync(self, days: int) -> str:
        ...
        try:
            logger.info("Cleaning up logs older than {days} days")
                return "sync_cleanup_{datetime.now().timestamp()}"
        except Exception as e:
...
from datetime import datetime
            ...
            logger.error("Backup task error: {e}")
    ...
    @celery_app.task(bind=True)
        ...
        try:
            ...
            logger.error("Optimization task error: {e}")
            self.update_state(state='FAILURE', meta={'error': str(e)})
                raise
    ...
    @celery_app.task(bind=True)
        ...
        try:
            ...
            self.update_state(state='PROGRESS', meta={'progress': 100})
                logger.info("Nutrition stats calculated for {date_range}")

            return {'status': 'success', 'stats': stats}
            except Exception as e:
            logger.error("Stats calculation task error: {e}")
            self.update_state(state='FAILURE', meta={'error': str(e)})
                raise
    ...
    @celery_app.task(bind=True)
        ...
        try:
            ...
            export_path = "exports/nutrition_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{export_format}"

            self.update_state(state='PROGRESS', meta={'progress': 100})
                logger.info("Data exported in {export_format} format")
    ...
    @celery_app.task(bind=True)
        ...
        try:
            ...
            self.update_state(state='PROGRESS', meta={'progress': 100})
                logger.info("Cleaned up logs older than {days} days")

            return {'status': 'success', 'cleaned_files': cleaned_files}
            except Exception as e:
            logger.error("Cleanup task error: {e}")
            self.update_state(state='FAILURE', meta={'error': str(e)})
                raise
    ...
    @celery_app.task(bind=True)
        ...
        try:
            ...
            self.update_state(state='PROGRESS', meta={'progress': 100})
                logger.info("Notification sent to user {user_id}: {message}")
