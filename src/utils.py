...
from .nutrition_calculator import calculate_calories_from_macros
    ...
    try:
        ...
        if result != result or result == float("in") or result == float("-in"):
            return default
            return result
...
def validate_nutrition_values(
    ...
    if check_total_macros:
        total_macros = protein + fat + carbs
            if total_macros > 100:
            errors.append("Total macros ({total_macros:.1f}g) cannot exceed 100g per 100g")
    ...
    # If calories are provided manually, check if they match calculated calories
        ...
        if abs(calories - calculated_calories) > 0.1:
                errors.append("Provided calories ({calories}) don't match calculated calories ({calculated_calories:.1f})")
...
def validate_dish_data(data: dict) -> tuple[bool, list, dict]:
    ...
    if not ingredients:
        ...
        for i, ingredient in enumerate(ingredients):
                ...
                errors.append("Ingredient {i+1}: Must be an object")
                continue
            ...
            if not product_id or product_id <= 0:
                    errors.append("Ingredient {i+1}: Valid product ID is required")
            elif quantity <= 0:
                errors.append("Ingredient {i+1}: Quantity must be greater than 0")
                elif quantity > 10000:
                errors.append("Ingredient {i+1}: Quantity cannot exceed 10000g")
            else:
                ...
                if preparation_method not in ["raw", "boiled", "steamed", "grilled", "fried", "baked"]:
                        errors.append("Ingredient {i+1}: Invalid preparation method '{preparation_method}'")
                    continue
                ...
                if edible_portion <= 0 or edible_portion > 1.0:
                    errors.append("Ingredient {i+1}: Edible portion must be between 0 and 1.0")
                        continue
...
def validate_log_data(data: dict) -> tuple[bool, list, dict]:
    ...
    if meal_time and meal_time not in valid_meal_times:
            errors.append("Meal time must be one of: {', '.join(valid_meal_times)}")
    else:
