...
class MetricsCollector:
    ...
    def _init_metrics(self):
        ...
        self.metrics["products_count"] = Gauge(
            "products_count", "Total number of products", registry=self.registry
        )

        self.metrics["dishes_count"] = Gauge(
            "dishes_count", "Total number of dishes", registry=self.registry
        )

        self.metrics["log_entries_count"] = Gauge(
            "log_entries_count", "Total number of log entries", registry=self.registry
        )
    ...
    def record_database_operation(self, operation: str, table: str, duration: float):
        ...
        if PROMETHEUS_AVAILABLE and "database_operations_total" in self.metrics:
            self.metrics["database_operations_total"].labels(operation=operation, table=table).inc()
    ...
    def record_fasting_session(self, status: str, fasting_type: str, duration: float = None):
        ...
        if PROMETHEUS_AVAILABLE and "fasting_sessions_total" in self.metrics:
            self.metrics["fasting_sessions_total"].labels(status=status, type=fasting_type).inc()

            if duration is not None and "fasting_duration_seconds" in self.metrics:
                self.metrics["fasting_duration_seconds"].labels(type=fasting_type).observe(duration)
    ...
    def record_background_task(self, task_type: str, status: str, duration: float = None):
        ...
        if PROMETHEUS_AVAILABLE and "background_tasks_total" in self.metrics:
            self.metrics["background_tasks_total"].labels(task_type=task_type, status=status).inc()

            if duration is not None and "task_duration_seconds" in self.metrics:
                self.metrics["task_duration_seconds"].labels(task_type=task_type).observe(duration)
...
def monitor_background_task(task_type: str):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                result = func(*args, **kwargs)
                metrics_collector.record_background_task(
                    task_type, "success", time.time() - start_time
                )
                return result
            except Exception:
                metrics_collector.record_background_task(
                    task_type, "failure", time.time() - start_time
                )
                raise
