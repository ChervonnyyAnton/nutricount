...
from flask import current_app
            ...
            user_agent = request.headers.get('User-Agent', '')
                identifier = "{client_ip}:{hash(user_agent) % 10000}"
...
class InputValidator:
    ...
    def log_auth_attempt(self, username: str, success: bool, ip: str, user_agent: str):
        ...
        self.logger.info(
                "AUTH_ATTEMPT - User: {username}, Status: {status}, "
            "IP: {ip}, User-Agent: {user_agent}"
        )

        def log_token_usage(self, user_id: int, action: str, ip: str):
        """Log token usage"""
        self.logger.info(
                "TOKEN_USAGE - User: {user_id}, Action: {action}, IP: {ip}"
        )

    def log_rate_limit_hit(self, identifier: str, limit_type: str, ip: str):
            """Log rate limit hit"""
        self.logger.warning(
            "RATE_LIMIT_HIT - Identifier: {identifier}, Type: {limit_type}, IP: {ip}"
            )

    def log_admin_action(self, user_id: int, action: str, ip: str):
            """Log admin action"""
        self.logger.info(
            "ADMIN_ACTION - User: {user_id}, Action: {action}, IP: {ip}"
            )
